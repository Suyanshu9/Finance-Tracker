...
std::vector<std::string>* readLabel(const char* labelToRead) {
    ...
    std::ifstream labelFile(labelToRead);  // Use C++ file stream

    if (!labelFile.is_open()) {  // Check if file opened successfully
        std::cerr << "Failed to open file: " << labelToRead << std::endl;
        return labels;
    }

    std::string line;
    while (std::getline(labelFile, line)) {  // Correct usage of std::getline
        labels->push_back(line);
    }

    labelFile.close();  // Close file after reading
    return labels;
}

double readBudget(const char* labelToRead) {
    std::ifstream budgetFile(labelToRead); // Use C++ file stream
    if (!budgetFile.is_open()) {  // Check if file opened successfully
        std::cerr << "Failed to open file: " << labelToRead << std::endl;
    ...
    std::string line;
    double budget = 0.0;
    while (std::getline(budgetFile, line)) { // Correct usage of std::getline
        budget = std::atof(line.c_str()); // Convert string to double
    }

    budgetFile.close(); // Close file after reading
    return budget;
}
